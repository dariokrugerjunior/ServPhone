...
function setBudgetAction(budget) {
	switch (budget.status) {
		...
		case 5: return `<i class="icon-pencil" style="cursor:pointer" onclick="window.location.assign('edit-budget.html?id=${budget.id}?status=${budget.status}')"></i>
						${sessionStorage.getItem('role') == 1 ? `<i style="cursor:pointer" class="icon-checkmark" data-toggle="tooltip" data-placement="top" title="Confirmar" onclick="updateStatus(${budget.id}, 9)"></i>` : ''}`
